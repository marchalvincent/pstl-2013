...
public class AlloyGenerator implements IAlloyGenerator {
	...
	@Override
	public List<IAlloyGenerated> generateFile() throws JetException {
		List<IAlloyGenerated> filesGenerated = new ArrayList<IAlloyGenerated>();
		...
		//TODO continuer avec l'executor
		// 2. On récupère les activités parsée
		List<Activity> activities = parser.getActivities();

		log.info("Début de la génération Alloy pour " + activities.size() + " activité(s).");
		...
		for (Activity activity : activities) {
			i++;
			pathFile = dirDestination + "gen_" + UMLFile.getName() + "_" + i + "_" + property.getClass().getSimpleName() + ".als";
			...
			try {
				...
				filesGenerated.add(Factory.getInstance().
						newAlloyGenerated(fichier, property.isCheck(), property.getStrategy()));
