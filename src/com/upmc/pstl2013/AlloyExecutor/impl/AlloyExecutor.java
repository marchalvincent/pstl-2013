	...
	public String executeFiles() throws Err, JetException {
		...
		for (File file : generator.getGeneratedFiles()) {
			try {
				...
				// TODO enlever
				// filename =
				// "D:\\INFORMATIQUE\\JAVA\\workspaces\\workspacePSTL\\pstl-2013\\model\\alloy\\GEN-simple.als";
				...
				if (filename
					...
					resultat.append("=========== Parsing+Typechecking "
							+ filename + " =============\n");

					Module world = CompUtil.parseEverything_fromFile(rep, null,
							filename);

					// Choose some default options for how you want to execute
					// the commands
					A4Options options = new A4Options();
					options.solver = A4Options.SatSolver.SAT4J; // TODO:
																// minisatx5 JNI
					...
					for (Command command : world.getAllCommands()) {
						...
						// If satisfiable...
						if (ans.satisfiable()) {
							// You can query "ans" to find out the values of
							// each set or type.
							// This can be useful for debugging.
							// You can also write the outcome to an XML file
							ans.writeXML("alloy_example_output.xml");
							// You can then visualize the XML file by calling
							// this:
							if (viz == null) {
								viz = new VizGUI(false,
										"alloy_example_output.xml", null);
							} else {
				...
				resultat.append("Impossible de récupérer le chemin du fichier : "
						+ e.toString() + "\n");
				log.error(
						"Impossible de récupérer le chemin du fichier : "
								+ e.toString(), e);
			}
		}
